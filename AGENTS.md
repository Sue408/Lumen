...
- **不要运行 `cargo fmt`，也不要引入 rustfmt。** 本项目是单人自用、按手写风格排版的，代码库并非 rustfmt 基线；跑一次 `cargo fmt` 会把大量无关行重排，产生成百上千行纯排版噪音，淹没功能改动。
- 新增 / 修改代码时**匹配周边既有手写风格**（折行位置、参数排布），不要顺手重排未改动的行。若确需格式化整仓，应作为**独立的纯格式提交**（不含功能改动），并在 `.git-blame-ignore-revs` 中登记，以免污染 `git blame`。
